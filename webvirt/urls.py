...
class VM:
	...
	if 'action' in data2.keys():
        ...
        content += "<input type=button onClick='window.location=tjhsst.edu/hackathon/vm?vm=" + vm + "&action=start' value='Start'>\
        <br><input type=button onClick='window.location=tjhsst.edu/hackathon/vm?vm=" + vm + "&action=stop' value='Stop'>\
        <br><input type=button onClick='window.location=tjhsst.edu/hackathon/vm?vm=" + vm + "&action=destroy' value='Destroy'>\
        <br><input type=button onClick='window.location=tjhsst.edu/hackathon/vm?vm=" + vm + "&action=suspend' value='Suspend'>\
        <br><input type=button onClick='window.location=tjhsst.edu/hackathon/vm?vm=" + vm + "&action=resume' value='Resume'>"
        data = ""
        for dom in conn.listAllDomains(0):
            dom = virt.Domain(dom)
            if(dom.rawstate == libvirt.VIR_DOMAIN_RUNNING):
